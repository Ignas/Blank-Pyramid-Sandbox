#!/usr/bin/make
#
# Makefile for Blank Project Sandbox
#
...
instance/var/run/.s.PGSQL.${PGPORT}:
	...
	${PG_PATH}/bin/pg_ctl -D ${PWD}/instance/var/data -o "-c unix_socket_directory=${PWD}/instance/var/run/ -c custom_variable_classes='blank' -c blank.active_user=0" start  -l ${PWD}/instance/var/log/pg.log
	sleep 5
...
.PHONY: clean
clean:
	rm -rf bin/ parts/ develop-eggs/ src/blank.egg-info/ python/ tags TAGS ID .installed.cfg
...
shell: bin/paster instance/done instance/var/run/.s.PGSQL.${PGPORT}
	bin/paster --plugin=Pyramid shell development.ini

export BUILD_ID ?= `date +%Y-%m-%d_%H-%M-%S`

.PHONY: package_release
package_release:
	git archive --prefix=blank${BUILD_ID}/ HEAD | gzip > blank${BUILD_ID}.tar.gz
